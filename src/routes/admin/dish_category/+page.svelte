<script lang="ts">
  ...
  onMount(async () => {
    categories = await dishCategoryRepo.getAll();
  });
  ...
  async function addDishCategory() {
    ...
    const newCategory = await dishCategoryRepo.add(categoryName.trim());
...
<main class="max-w-3xl mx-auto p-4 space-y-6 text-gray-700 dark:text-gray-100">
  <h3 class="text-center text-2xl font-semibold">Категории блюд</h3>
